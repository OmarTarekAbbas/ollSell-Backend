...
class DropshipperRepository extends BasicRepository
{
    /**
     * @var array
     */
    protected $fieldSearchable = [
        'id', 'status',  'email', 'isVerified', 'first_name', 'second_name','created_at'
    ];
     ...
     */
    public function findBy(Request $request, $pagination = false, $perPage = 4, $orderBy = [], $moreConditionForFirstLevel = [], $recursiveRel = [], $get = '', $limit = null,$withRelations=[])
    {
        return $this->all($request->all(), pagination: $pagination, perPage: $perPage, orderBy: $orderBy, moreConditionForFirstLevel: $moreConditionForFirstLevel, recursiveRel: $recursiveRel, get: $get, limit: $limit,withRelations:$withRelations);
    }
    ...
    public function save(Request $request, $id = null)
    {
        return DB::transaction(function () use ($request, $id) {
            ...
            //todo change
            if (isset($request->target_market)) {
                $data->targetMarket()->sync((array)$request->target_market);
            }else {
                $data->targetMarket()->sync([1, 2, 3]);
            }
            if (isset($request->questionnaire_target_markets)) {
                $data->onboarding_questionnaire_target_markets()->sync((array)$request->questionnaire_target_markets);
            }
            if (isset($request->questionnaire_category)) {
                $data->onboarding_questionnaire_category()->sync((array)$request->questionnaire_category);
            }
            if (isset($request->questionnaire_dropshipper_social)) {
                $data->onboarding_questionnaire_dropshipper_social()->sync((array)$request->questionnaire_dropshipper_social);
            }
