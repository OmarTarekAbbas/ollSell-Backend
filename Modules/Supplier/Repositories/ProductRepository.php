...
class ProductRepository extends BasicRepository
{
    ...
    protected $fieldSearchable = [
        'id', 'status','warehouse_id', 'cost_price',  'sku', 'size', 'is_discount', 'saleCountProduct', 'is_recommended', 'priceAfterDiscount', 'created_at', 'quantity', 'weight','isApproved'
    ];
        ...
        return DB::transaction(function () use ($request, $id) {
            if ($id) {
                ...
                if ($request->thumbnail) {
                    $data->thumbnail()->delete();
                }
            } else {
                $data = $this->create($request->all());
            }
            $data->targetMarket()->sync($request->target_market_id);
            ...
            // $this->media_upload($data, $request, createFileNameServer($this->model(), $data->id), pathType()['ip'], mediaType()['bc']);
            $this->media_upload($data, $request, createFileNameServer($this->model(), $data->id), pathType()['ip'], mediaType()['th']);
            return $data;
        });
    }
    ...
    public function list(Request $request, $pagination = true, $perPage = 20, $moreConditionForFirstLevel = [], $recursiveRel = [])
    {
        ...
        return $this->all(search: $request->all(), orderBy: ['column' => 'id', 'order' => 'desc'], pagination: $pagination, perPage: $perPage, moreConditionForFirstLevel: $moreConditionForFirstLevel, recursiveRel: $recursiveRel);
    }
    public function chunkProducts(Request $request)
    {

        return   Product::join('product_target_markets', 'product_target_markets.product_id', '=', 'products.id')->whereIn('product_target_markets.target_market_id', $request->target_market)
            ->select('products.*')->get()
            ->chunk(300);
    }
    ...
    {
        $data = $this->findOne($request->product_id);

        $image =   $data->logo()->where('id', $request->image_id)->first();
        $path = getFileWithoutURL($image->file, 'images', getFileNameServer($image));
        File::delete($path);
        $image->delete();
    }
}
