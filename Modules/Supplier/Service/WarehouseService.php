...
class WarehouseService extends BasicService
{
    ...
    public function index(Request $request)
    {
        ...
        if(isset($request->is_report) && $request->is_report)
        {
            ...
            }elseif($request->fromDate)
            {
                $recursiveRel = [
                    'product' => [
                        'type' => 'whereHas',
                        'where' => ['created_at' => ['>', Carbon::parse($request->fromDate)->startOfDay()],'supplier_id' => user()->id]
                    ]
                ];
            }elseif($request->toDate)
            {
                $recursiveRel = [
                    'product' => [
                        'type' => 'whereHas',
                        'where' => ['created_at' => ['<', Carbon::parse($request->toDate)->endOfDay()],'supplier_id' => user()->id]
                    ]
                ];
            }
        }
        ...
        return $this->repo->findBy($request, pagination: true, moreConditionForFirstLevel: $moreConditionForFirstLevel, perPage: $tableLength, orderBy: ['column' => 'id', 'order' => 'desc'],recursiveRel:$recursiveRel);
    }
    ...
    public function list(Request $request)
    {
        ...
        return $this->repo->list(new Request(),moreConditionForFirstLevel:$moreConditionForFirstLevel,pagination:false);
    }
}
