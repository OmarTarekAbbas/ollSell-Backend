...
<div id="kt_app_content" class="app-content flex-column-fluid">
    ...
    <div class="card card-flush">
        ...
        <div class="card-header border-0 pt-6">
                ...
                <div class="d-flex align-items-center position-relative my-1">
                    ...
                    <span class="svg-icon svg-icon-1 position-absolute ms-6">
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                ...
                                <rect opabundles="0.5" x="17.0365" y="15.1223" width="8.15546" height="2" rx="1"
                    ...
                    <!--end::Svg Icon-->
                    <input id="search" type="text" data-kt-bundles-table-filter="search"
                           class="form-control form-control-solid w-250px ps-15" placeholder="Search bundles" value="{{request()->search}}"/>
                </div>
                <!--end::Search-->
            </div>
            <!--begin::Card title-->
            ...
            <div class="card-toolbar">
                <!--begin::Toolbar-->
                    ...
                    <!--begin::Add customer-->
                    <a href="{{ route('bundles.create') }}" class="btn btn-primary"> Add bundle</a>
                    <!--end::Add customer-->
                    @endpermission
                </div>
        ...
        <div class="card-body pt-0" id="main_table">
            @include('mastercatalog::bundle.table')
        </div>
    </div>
</div>
        ...
        @push('scripts')
            <script>
                var routeAll = "{{ route('bundles.index', Request()->all()) }}";
                var route = "{{ route('bundles.index') }}";
                var toggleActiveRoute = "{{ route('bundles.changeStatus') }}";
                var csrfToken = "{{ csrf_token() }}";
                var deletePermission = {{permissionShow('delete_bundle') ? 1 : 0}};
                var updatePermission = {{permissionShow('update_bundle') ? 1 : 0}};
            </script>
            <script src="{{ asset('dashboard') }}/assets/js/bundles/list.js?v=2122"></script>
            ...
            <script>
                $(document).ready(function () { 
                    ...
                    // update URL 
                    full = routeAll;
                    if(full.substring(full.lastIndexOf('/') + 1) == 'bundles'){
                ...
                function handleDeleteRows() {
                    ...
                    const deleteButtons = document.querySelectorAll(
                        '[data-kt-bundles-table-filter="delete_row"]'
                    );
                    if (deleteButtons) {
