...
@section('content')
    ...
    <div class="card card-flush">
        ...
        <div class="card-header border-0 pt-6">
            ...
            <div class="card-title">
                ...
                <div class="d-flex align-items-center position-relative my-1">
                    ...
                    <!--end::Svg Icon-->
                    <input id="search" type="text" data-kt-event-table-filter="search"
            ...
            <div class="card-toolbar flex-row-fluid justify-content-end gap-5 d-flex">
                <div class="w-100 mw-150px">
                    ...
                    <select id="status" class="form-select form-select-solid" data-control="select2"
                        data-hide-search="true" data-placeholder="Status" data-kt-ecommerce-event-filter="status">
        ...
        <div class="card-body pt-0" id="main_table">
            @include('mastercatalog::event.table')
        </div>
        <!--end::Datatable-->
    </div>
        </div>
...
@push('scripts')
    <script>
        var routeAll = "{{ route('event.index', Request()->all()) }}";
        var route = "{{ route('event.index') }}";
        var toggleActiveRoute = "{{ route('event.changeStatus') }}";
        var csrfToken = "{{ csrf_token() }}";
        var deletePermission = {{ permissionShow('delete_event') ? 1 : 0 }};
        var updatePermission = {{ permissionShow('update_event') ? 1 : 0 }};
    </script>
    <script src="{{ asset('dashboard') }}/assets/js/event/list.js?v=1"></script>

    <script>
        $('#search').on('keyup', function() {
            // update URL 
            full = routeAll;
            if (full.substring(full.lastIndexOf('/') + 1) == 'event') {
    ...
    <script>
        ...
        function handleDeleteRows() {
            ...
            const deleteButtons = document.querySelectorAll(
                '[data-kt-event-table-filter="delete_row"]'
            );
            if (deleteButtons) {
