...
use Modules\MasterCatalog\Actions\Product\CreateAndUpdateWMSAction;
...
class ProductRepository extends BasicRepository
{
    ...
    public function save(Request $request, $id = null)
    {
        return DB::transaction(function () use ($request, $id) {
            ...
            $data->attributes()->sync($request['attributes']);
            $data->events()->sync($request['events']);
            $data->related_product()->sync($request['related_products']);
            $data->product_dropshipper()->sync($request['product_dropshippers']);
            $this->updateOrCreateLanguage($data, $request, $this->translationKey());
            $this->media_upload(
                $data,
                $request,
                createFileNameServer($this->model(), $data->id),
                pathType()['ip'],
                mediaType()['lm']
            );
            ...
            if ($id) {
                App(CreateAndUpdateWMSAction::class)->update($data);
            } else {
                App(CreateAndUpdateWMSAction::class)->create($data);
            }
    ...
    {
        return Product::join('product_target_markets', 'product_target_markets.product_id', '=', 'products.id')
            ->whereIn('product_target_markets.target_market_id', $request->target_market)
            ->select('products.*')->get()
            ->chunk(300);
    }
