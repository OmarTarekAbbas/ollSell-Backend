...
class ProductVariantValueRepository extends BasicRepository
{
    /**
     * @var array
     */
    protected $fieldSearchable = [
        'id', 'product_variant_id', 'attribute_option_id', 'attribute_id', 'product_id'
    ];
    ...
    public function save($data, $id = null)
    {
        return DB::transaction(function () use ($data, $id) {
            if ($id) {
                $respose = $this->update($data, $id);
            } else {
                $respose = $this->create($data);
            }
            return $respose;
        });
    }
    ...
    {//todo change
        ...
        return $this->all(search: $request->all(), orderBy: ['column' => 'id', 'order' => 'asc'], pagination: $pagination, perPage: $perPage, moreConditionForFirstLevel: $moreConditionForFirstLevel, recursiveRel: $recursiveRel);
    }
}
