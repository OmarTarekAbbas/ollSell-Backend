...
class OrderItemService extends BasicService
{
    ...
    {
        return (new CreateOrderItemAction(
            order: $order
        ))->execute();
    }
    ...
    /**
     * The function updates the status of all order items associated with a given order to match the
     * order's status.
     *
     * param order The `updateStatusOrderItem` function takes an `` object as a parameter. This
     * function updates the `status_id` of all order items associated with the given order.
     */
    public function updateStatusOrderItem($order)
    {
        $orderItems = OrderItem::where('order_id', $order->id)->get();
        foreach ($orderItems as $orderItem) {
            $o = Order::find($orderItem->order_id);
            $orderItem->status_id = $o->status_id;
            $orderItem->save();
        }
    }
}
