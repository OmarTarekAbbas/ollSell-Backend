...
class AymakanCreateShipmentOrderAction
{
    protected $repo;
     ...
     * will return false.
     */
    public function execute($requestAymakan)
    {
        try {
            $result = Aymakan::createShipment($requestAymakan);
        } catch (\Exception $e) {
            return $e->getMessage();
        }
        return $result;
    ...
    public function getOrderByReference($order)
    {
        $trackShipment = Http::withHeaders([
            'Accept' => 'application/json',
            'authorization' => config('services.aymakan.AYMAKAN_SECRET_API_KEY')
        ])->get(config('services.aymakan.AYMAKAN_API_URL') . '/shipping/by_reference/' . $order->id);
        ...
        foreach (array_reverse($trackingInfos) as $key => $trackingInfo) {
            ...
            } elseif (in_array($trackingInfo['status_code'], [
                'AY-0026',
                'AY-0009',
                'AY-0030',
                'AY-0069',
