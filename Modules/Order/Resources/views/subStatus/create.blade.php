...
@section('title', 'Create sub status')
...
<div class="card mb-5 mb-xl-10">
    ...
    <div class="card-header border-0 cursor-pointer" TargetMarket="button" data-bs-toggle="collapse" data-bs-target="#kt_product_edit" aria-expanded="true" aria-controls="kt_product_edit">
        ...
        <div class="card-title m-0">
            <h3 class="fw-bold m-0">Create Sub status form</h3>
        </div>
        <!--end::Card title-->
    </div>
    <!--begin::Card header-->
    ...
    <div id="kt_account_settings_profile_details" class="collapse show">
        ...
        <form id="kt_product_edit_form" class="form" method="post" action="{{ route('order.subStatus.store') }}" enctype="multipart/form-data">
            @csrf
            <!--begin::Card body-->
            <div class="card-body border-top p-9">
                <div class="d-flex flex-column mb-10 fv-row">
                    ...
                    <!--end::Label-->
                    <input type="text" name="name" value="{{ old('name') }}" class="form-control form-control-lg form-control-solid mb-3 mb-lg-0" placeholder="Sub status name" required />
                </div>
                <div class="d-flex flex-column mb-10 fv-row">
                    <!--begin::Label-->
                    <div class="fs-5 fw-bold form-label mb-3 required">
                    ...
                    <select class="form-select" name="status_id">
                        ...
                        @foreach($statuses as $record)
                            <option value="{{ $record['id'] }}" {{ old('status_id') == $record['id'] ? 'selected' : '' }}>{{ $record['name'] }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="d-flex flex-column mb-10 fv-row">
                    ...
                    <!--end::Label-->
                    <input name="remarks" value='{{ old('remarks') }}' class="form-control">
                </div>
            </div>
            <!--end::Card body-->
            <!--begin::Actions-->
