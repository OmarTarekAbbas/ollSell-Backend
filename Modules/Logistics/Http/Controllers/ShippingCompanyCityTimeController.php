...
use Modules\Logistics\Http\Requests\ShippingCompanyCityTime\CreateRequest;
use Modules\Logistics\Http\Requests\ShippingCompanyCityTime\EditRequest;
use Modules\Logistics\Service\ShippingCompanyCityTimeService;
use Modules\CoreData\Service\RegionService;
class ShippingCompanyCityTimeController extends BasicController
{
    protected $service;
    protected $regionService;
    /**
     * This function sets up middleware and assigns a ShippingCompanyCityTimeServiceService object to a class property.
     * 
     * param ShippingCompanyCityTimeServiceService Service An instance of the ShippingCompanyCityTimeServiceService class, which is likely responsible for
     * handling business logic related to shipping_company_city_times (e.g. retrieving, creating, updating, and deleting
     * city data).
     */
    public function __construct(ShippingCompanyCityTimeService $Service, RegionService $regionService)
    {
   
        $this->middleware('auth');
        $this->middleware('permission:view_shipping_company_city_times')->only('index');
        $this->middleware('permission:create_shipping_company_city_times')->only(['create','store']);
        $this->middleware('permission:update_shipping_company_city_times')->only(['edit','update']);
        $this->middleware('permission:delete_shipping_company_city_times')->only('destroy');
    ...
    public function index(Request $request)
    {
        ...
        $shipping_company_city_times = $this->service->index($request);
        if ($request->ajax()) {
            return view('logistics::shipping_company_city_time.table')->with(['shipping_company_city_times' => $shipping_company_city_times]);
        }

        return $this->getDashboardView('logistics::shipping_company_city_time.index', compact('shipping_company_city_times'));
    }
    ...
    public function create()
    {
        ...
        $cities = $this->regionService->cityList();
        return $this->getDashboardView('logistics::shipping_company_city_time.create', compact('shippingcompanies','cities'));
    }
    ...
    public function store(CreateRequest $request)
    {
        ...
        if ($data) {
            return redirect(route('shipping_company_city_time.index'))->with('Done');
        }
        return redirect(route('shipping_company_city_time.create'))->with('problem');
    }
     ...
     * 
     * return a view named "logistics::shipping_company_city_time.edit" with the data and shippingcompanies passed as compact
     * variables.
     */
   ...
   $shippingcompanies = $this->service->shippingCompanyList();
        ...
        return $this->getDashboardView('logistics::shipping_company_city_time.edit', compact('data', 'cities','shippingcompanies'));
    }
    ...
    public function update(EditRequest $request, $id)
    {
        ...
        if ($data) {
            return redirect(route('shipping_company_city_time.index'))->with("message", 'Done');
        }
        return redirect(route('shipping_company_city_time.edit', $id))->with('problem');
    }
}
