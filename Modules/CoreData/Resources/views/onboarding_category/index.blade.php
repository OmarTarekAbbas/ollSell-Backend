...
<div id="kt_app_content" class="app-content flex-column-fluid">
    ...
    <div class="card card-flush">
        ...
        <div class="card-header border-0 pt-6">
            <div class="card-title">
                ...
                <div class="d-flex align-items-center position-relative my-1">
                    ...
                    <input type="text"
                           ...
                           placeholder="Search Onboarding Category"
            ...
            <div class="card-toolbar">
                <div class="d-flex justify-content-end" data-kt-customer-table-select="base">
                    @permission('create_onboarding_categories')
                    <!--begin::Add customer-->
                    <a href="{{ route('onboarding_category.create') }}" class="btn btn-primary"> Add Onboarding
                        Category</a>
                    <!--end::Add customer-->
                    @endpermission
                </div>
        ...
        <div class="card-body pt-0" id="main_table">
            @include('coredata::onboarding_category.table')
        </div>
    </div><!--end::Products-->
  </div>
...
@push('scripts')
    <script>
        let route = "{{ route('onboarding_category.index') }}";
        let routeAll = "{{ route('onboarding_category.index',Request()->all()) }}";
        let csrfToken = "{{ csrf_token() }}";
        let deletePermission = {{permissionShow('delete_onboarding_categories') ? 1 : 0}};
        let updatePermission = {{permissionShow('update_onboarding_categories') ? 1 : 0}};
        let toggleActiveRoute = "{{ route('onboarding_category.changeStatus') }}";
    </script>
    <script src="{{ asset('dashboard') }}/assets/js/onboarding_category/list.js?v=12"></script>
    ...
    <script>
        ...
        function handleDeleteRows() {
            ...
            const deleteButtons = document.querySelectorAll(
                '[data-kt-onboarding-category-table-filter="delete_row"]'
            );
            if (deleteButtons) {
