...
class DropshipperMappingProductRepository extends BasicRepository
{
    /**
     * @var array
     */
    protected $fieldSearchable = [
        'dropshipper_id', 'model_type', 'model_id', 'product_id', 'move'
    ];
    ...
    {
        return DropshipperMappingProduct::class;
    }
    ...
    public function save(Request $request)
    {
        return DB::transaction(function () use ($request) {
            //todo change
         $dropmapping  = DropshipperMappingProduct::where('dropshipper_id',$request->dropshipper_id)
            ->where('model_type','salla')->where('model_id',$request->model_id)->first();
       
            if ($dropmapping) {
                $data = $this->update($request->all(), $dropmapping->id);
            
            } else {
                $data = $this->create($request->all());
            }
            return isset($dropmapping) ?  $dropmapping : $data;
        });
    }
}
